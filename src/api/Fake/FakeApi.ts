...
async function getFromStorage(): Promise<IndexedDictionary<Product>> {
  ...
  if (res == null) await updateStorage();
  else products = new IndexedDictionary<Product>('id', JSON.parse(res));
...
export default class FakeApi implements IProductsApi {
  ...
  async updateProduct(product: Product): Promise<void> {
    if (!products.ContainsIndex(product.id)) return Promise.reject('ID not found');
